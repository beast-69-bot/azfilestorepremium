...
def _welcome_text() -> str:
    return (
        ...
        "• [b]Premium VIP Access[/b]: Premium users bypass all force-joins, get higher daily link quotas, and enjoy direct ad-free delivery.\n\n"
        "📌 [b]How to Get Files?[/b]\n"
        "1️⃣ Click on any deep-link generated by the admin.\n"
...
async def _get_payment_gateway(db: Database, cfg: Any) -> str:
    val = await db.get_setting("payment_gateway")
    if val:
        return val.strip().lower()
    return getattr(cfg, "payment_gateway", "manual")


async def _get_xwallet_api_key(db: Database, cfg: Any) -> str:
    val = await db.get_setting("xwallet_api_key")
    if val:
        return val.strip()
    return getattr(cfg, "xwallet_api_key", "")


async def _get_razorpay_key_id(db: Database, cfg: Any) -> str:
    val = await db.get_setting("razorpay_key_id")
    if val:
        return val.strip()
    return getattr(cfg, "razorpay_key_id", "")


async def _get_razorpay_key_secret(db: Database, cfg: Any) -> str:
    val = await db.get_setting("razorpay_key_secret")
    if val:
        return val.strip()
    return getattr(cfg, "razorpay_key_secret", "")
...
_USER_UPSERT_CACHE: dict[int, tuple[str | None, str | None, float]] = {}
USER_UPSERT_CACHE_TTL = 300  # 5 minutes

_FORCE_JOIN_MEMBER_CACHE: dict[tuple[int, int], tuple[bool, float]] = {}
FORCE_JOIN_CACHE_TTL_SUCCESS = 180  # 3 minutes for joined status
FORCE_JOIN_CACHE_TTL_FAIL = 3        # 3 seconds for not joined status
...
async def _upsert_user(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if cached:
        cached_first, cached_user, cached_ts = cached
        if cached_first == first_name and cached_user == username and (now - cached_ts < USER_UPSERT_CACHE_TTL):
            return  # Skip database upsert to reduce writes

    db: Database = context.application.bot_data["db"]
    _USER_UPSERT_CACHE[user_id] = (first_name, username, now)
...
async def _joined_all_force_channels_details(
    ...
    async def _check_channel(ch: dict[str, Any]) -> tuple[bool, dict[str, Any], dict[str, Any]]:
        ...
        # Check in-memory cache for chat membership status
        now_ts = time.time()
        cached_status = _FORCE_JOIN_MEMBER_CACHE.get((user_id, cid))
        if cached_status:
            c_joined, c_ts = cached_status
            ttl = FORCE_JOIN_CACHE_TTL_SUCCESS if c_joined else FORCE_JOIN_CACHE_TTL_FAIL
            if now_ts - c_ts < ttl:
                is_joined = c_joined
                # We can skip the Telegram API call if membership is cached
                passed = is_joined
                if mode == "request":
        ...
        # Update cache
        _FORCE_JOIN_MEMBER_CACHE[(user_id, cid)] = (is_joined, time.time())
...
async def cancel(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # Active payment request (QR flow) cleanup
    db: Database = context.application.bot_data["db"]
    try:
        open_req = await db.get_latest_open_payment_request(update.effective_user.id)
    except Exception:
        open_req = None
    if open_req and str(open_req.get("status") or "") in ("pending", "submitted"):
        try:
            await db.expire_payment_request_if_pending(int(open_req["id"]))
            await _update_payment_user_status(
                ...
                [
                    ...
                    "This order was closed through /cancel.",
...
def _payment_plan_label(plan_key: str, plan_days: int) -> str:
...
async def _handle_manual_payment_recovery(
    update: Update,
    context: ContextTypes.DEFAULT_TYPE,
    q: Any,
    rid: int,
    ...
    if not upi_id:
        await _edit_emoji_text(
            update.effective_chat.id,
            q.message.message_id,
            "Payment is not configured by admin yet.\nPlease contact admin.",
            context,
        )
        return

    req = await db.get_payment_request(int(rid))
    if not req or not update.effective_user or not update.effective_chat:
        await _send_emoji_text(update.effective_chat.id, "Payment request expired/invalid. Please run /pay again.", context)
        return

    projected_until = int(req.get("projected_premium_until") or 0)
    note = _manual_payment_note(update.effective_user.id, rid, int(plan["days"]), projected_until)
    upi_uri = _upi_uri(upi_id=upi_id, amount_rs=int(plan["amount"]), payee_name=pay_name, note=note)
    qr_url = _upi_qr_image_url(upi_uri)
    plan_label = html.escape(str(plan["label"]))
    upi_html = html.escape(upi_id)
...
async def _send_premium_direct_access_message(user_id: int, context: ContextTypes.DEFAULT_TYPE) -> None:
    db: Database = context.application.bot_data["db"]
    channels_text = await _get_premium_channels_text(db)
    channels_section = (
    ...
    await _send_emoji_text(user_id, text, context, disable_web_page_preview=True)
...
async def _handle_razorpay_payment(update: Update, context: ContextTypes.DEFAULT_TYPE, q: Any, rid: int, plan: dict, cfg: Any) -> None:
    ...
    try:
        ...
        key_secret = await _get_razorpay_key_secret(db, cfg)
...
async def pay_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if data.startswith("payplan:"):
        ...
        if selected_gate == "stars":
            ...
            is_stars = False
            gateway = await _get_payment_gateway(db, cfg)
            is_xwallet = gateway == "xwallet" and bool(await _get_xwallet_api_key(db, cfg))
            is_razorpay = gateway == "razorpay" and bool(await _get_razorpay_key_id(db, cfg)) and bool(await _get_razorpay_key_secret(db, cfg))
            ...
            if existing.get("status") == "pending":
                ...
                if changed:
                    await _update_payment_user_status(
                        ...
                        [
                            "Status: <b>Expired</b>",
                            "A newer order was created for this user.",
...
async def pay_admin_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if action == "approve":
        ...
        await _update_manual_payment_user_final_status(
            ...
            [
                "Status: <b>Approved</b>",
                f"Premium Until: <code>{html.escape(expiry_utc)}</code>",
                f"Approved By: <code>{html.escape(admin_name)}</code>",
            ],
        )
        await db.clear_payment_ui_messages(int(req["id"]))
        return
    ...
    await _update_manual_payment_user_final_status(
        ...
        [
            "Status: <b>Rejected</b>",
            f"Reviewed At: <code>{html.escape(reviewed_at)}</code>",
            "If this payment was real, admin can still recover it later from bank history.",
        ],
    )
...
async def paylookup(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    projected_until = int(req.get("projected_premium_until") or 0)
    trace_note = _manual_payment_note(
        int(req["user_id"]),
        int(req["id"]),
        int(req.get("plan_days") or 0),
        projected_until,
    )
...
async def manualapprove(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await _update_manual_payment_user_final_status(
        ...
        [
            ...
            f"Premium Until: <code>{html.escape(expiry_utc)}</code>",
            f"Approved By: <code>{html.escape(admin_name)}</code>",
            "This payment was recovered from transaction history.",
        ],
    )
    await db.clear_payment_ui_messages(int(req["id"]))
    await _send_emoji_text(
...
async def setpay(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not context.args:
        ...
        await _send_emoji_text(
            ...
            context,
            reply_markup=_bset_setpay_keyboard(gateway),
        )
        return
    ...
    if sub == "view":
        ...
        gateway = await _get_payment_gateway(db, cfg)
        xwallet_key = await _get_xwallet_api_key(db, cfg)
        razorpay_id = await _get_razorpay_key_id(db, cfg)
        razorpay_sec = await _get_razorpay_key_secret(db, cfg)
        ...
        await _send_emoji_text(
            ...
            f"• Payee Name: {name}\n"
            f"• Instructions: {text}\n"
            f"• Active Gateway: [b]{gateway}[/b]\n"
            f"• XWallet Key: [c]{mask(xwallet_key)}[/c]\n"
            f"• Razorpay Key ID: [c]{mask(razorpay_id)}[/c]\n"
            f"• Razorpay Secret: [c]{mask(razorpay_sec)}[/c]",
            context
        )
        return
    if sub == "clearupi":
    ...
    if sub == "gateway":
        if len(context.args) < 2:
            await _send_emoji_text(update.effective_chat.id, "❌ Usage: /setpay gateway <manual|xwallet|razorpay>", context)
            return
        g = context.args[1].strip().lower()
        if g not in ("manual", "xwallet", "razorpay"):
            await _send_emoji_text(update.effective_chat.id, "❌ Invalid gateway. Choose manual, xwallet, or razorpay.", context)
            return
        await db.set_setting("payment_gateway", g)
        await _send_emoji_text(update.effective_chat.id, f"✅ Payment gateway set to: [b]{g}[/b]", context)
        return
    if sub == "xwalletkey":
        if len(context.args) < 2:
        ...
        await _send_emoji_text(update.effective_chat.id, "✅ XWallet API Key updated.", context)
        return
    if sub == "razorpayid":
        if len(context.args) < 2:
        ...
        await _send_emoji_text(update.effective_chat.id, "✅ Razorpay Key ID updated.", context)
        return
    if sub == "razorpaysecret":
        ...
        await _send_emoji_text(update.effective_chat.id, "✅ Razorpay Key Secret updated.", context)
        return
...
async def bsettings_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if data == "bset:setpay_panel":
        ...
        return
    if data.startswith("bset:setpay_mode_"):
        mode = data.replace("bset:setpay_mode_", "")
        db: Database = context.application.bot_data["db"]
        cfg = context.application.bot_data["cfg"]
        await db.set_setting("payment_gateway", mode)
        gateway = await _get_payment_gateway(db, cfg)
        await _edit_emoji_text(
            update.effective_chat.id,
            q.message.message_id,
            f"✅ Payment gateway set to: [b]{mode}[/b]\n\nChoose action below:",
            context,
            reply_markup=_bset_setpay_keyboard(gateway),
        )
        return
    ...
    if data == "bset:setpay_clearupi":
        ...
        return
    if data in ("bset:setpay_upi", "bset:setpay_name", "bset:setpay_text", "bset:setpay_xwalletkey", "bset:setpay_razorpayid", "bset:setpay_razorpaysecret"):
        mode = data.replace("bset:setpay_", "")
        ...
        prompts = {
            ...
            "xwalletkey": "🎟 XWallet API Key bhejo",
            "razorpayid": "💳 Razorpay Key ID bhejo",
            "razorpaysecret": "🔑 Razorpay Key Secret bhejo",
        }
        await _edit_emoji_text(
            update.effective_chat.id,
            q.message.message_id,
...
async def bsettings_misc_input(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if mode:
        if mode == "upi":
            ...
            await db.set_setting(SETTINGS_PAY_TEXT, text)
            await _send_emoji_text(update.effective_chat.id, "✅ Payment text set.", context)
        elif mode == "xwalletkey":
            await db.set_setting("xwallet_api_key", text)
            await _send_emoji_text(update.effective_chat.id, "✅ XWallet API Key updated.", context)
        elif mode == "razorpayid":
            await db.set_setting("razorpay_key_id", text)
            await _send_emoji_text(update.effective_chat.id, "✅ Razorpay Key ID updated.", context)
        elif mode == "razorpaysecret":
            await db.set_setting("razorpay_key_secret", text)
            await _send_emoji_text(update.effective_chat.id, "✅ Razorpay Key Secret updated.", context)
        context.user_data.pop("bset_setpay_wait", None)
        return
    ...
    for req in pending_requests:
        ...
        if gateway == "razorpay" and req.get("plan_key"):
            ...
            if qr_code_id:
                ...
                class SimpleContextRazorpay:
                    def __init__(self, app: Application):
                        self.application = app
                        self.bot = app.bot

                dummy_context = SimpleContextRazorpay(application)
        ...
        elif gateway == "xwallet" and req.get("plan_key"):
            api_key = await _get_xwallet_api_key(db, cfg)
            if not api_key:
                continue
            ...
            if qr_code_id:
                logger.info("Resuming polling for XWallet pending request %d (QR: %s)", rid, qr_code_id)
                class SimpleContextXWallet:
                    def __init__(self, app: Application):
                        self.application = app
                        self.bot = app.bot

                dummy_context = SimpleContextXWallet(application)
                asyncio.create_task(_poll_and_complete(
                    context=dummy_context,
                    rid=rid,
                    qr_code_id=qr_code_id,
                ))
...
def build_handlers(app: Application) -> None:
    ...
    app.add_handler(CommandHandler("paylookup", paylookup))
    app.add_handler(CommandHandler("manualapprove", manualapprove))
